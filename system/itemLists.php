	...
	try{
		...
		if($stmt->execute()){
			...
			foreach($categories as $category){
				$query = "SELECT * FROM ".$category->Tables_in_materials;
				$stmt2 = $db->prepare($query);
						 $stmt2->execute();
				$results = $stmt2->fetchAll(PDO::FETCH_OBJ);
				
				echo "<div id=".($category->Tables_in_materials)." name='".$category->Tables_in_materials."'>";
				//echo "<section name='".$category->Tables_in_sethichatbhandar2."' id='#".$category->Tables_in_sethichatbhandar2."'>";
				echo "<h3><i>".$category->Tables_in_materials."</i></h3>";
				if(empty($results)){
					echo "No Items <br><br>";
				}
